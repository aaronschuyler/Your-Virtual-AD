      ...
      <div class="team" v-for="(team, index) in teams" :key="team.teamName">
        <table v-if="team.display">
          <tr>
            ...
            <td>
              <button class="theme-button" @click="toggle(index)">
                <span v-if="teams[index].published">Unpublish</span>
          ...
          <tr v-if="addGame[index]" class="add">
            ...
            <td>
              <select @change="getGymsForVS()" name="vs" v-model="newGameVS">
                ...
                <option
                  v-for="vs in vss"
                  v-bind:value="vs.orgName"
                  v-bind:key="vs.orgName"
                  >{{ vs.orgName }}</option
                >
              </select>
            </td>
            <td>
              <select @change="getGymsForVS()" v-model="home">
                <option value="null" disabled selected>Home or Away</option>
...
export default {
  ...
  methods: {
    ...
    async getOrg() {
      ...
      this.orgGyms = response.data[0].gymLocations;
    },
    async getOrgs() {
      const response = await OrgService.fetchOrgs();
      this.vss = response.data.orgs;
    },
    async addTeam() {
    ...
    async getGymsForVS() {
      ...
      var filtered = [];
      const response = await OrgService.getOrg({
        orgName: this.newGameVS
      });
      if (this.home == "true") {
        if (
          this.$route.params.sport == "Basketball" ||
          this.$route.params.sport == "Volleyball"
        ) {
          var type = "court/gym";
        } else {
          type = "field";
        ...
        for (var i = 0; i < this.orgGyms.length; i++) {
          gss.push(this.orgGyms[i]);
        }
      }
      if (this.home == "false") {
        if (
        ...
        gss.push({
          gymName: this.newGameVS + " " + this.$route.params.sport + " " + type,
          sport: this.$route.params.sport
        });
        for (i = 0; i < response.data[0].gymLocations.length; i++) {
          gss.push(response.data[0].gymLocations[i]);
        }
      }
      for (i = 0; i < gss.length; i++) {
        if (
          this.$route.params.sport == gss[i].sport &&
          !filtered.some(e => e.gymName === gss[i].gymName)
        ) {
          filtered.push(gss[i]);
        }
      }
    ...
    async toggle(i) {
      if (this.teams[i].published == true) {
